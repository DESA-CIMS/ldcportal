...
AUI().use('get', function(A){     
	A.Get.script('http://code.jquery.com/jquery-1.7.1.min.js', {
    	onSuccess: function() {
           	$(window).load(function() {	         		
 	
	           		$('div.special-un-column div.maximize-portlet').each(function() {
	           			$(this).parents('div.portlet-asset-publisher').append(this);
	           		});
	           		
	           		//if ($.browser.msie) {
	           			var maxHeight1 = 0;
	           			var maxHeight2 = 0;
	           			$('div.special-un-column div.portlet-asset-publisher').each(function(index) {
	           				var height = $(this).css("height").slice(0, -2);
	           				if (index < 3) {
		           				if (parseInt(maxHeight1) < parseInt(height)) {
		           					maxHeight1 = height;
		           				}
	           				}
	           				else if (parseInt(maxHeight2) < parseInt(height)) {
	           					maxHeight2 = height;
	           				}
		           		});
		           		$('div.special-un-column div.portlet-asset-publisher').each(function(index) {
		           			if (index < 3) {
		           				$(this).css("height", maxHeight1 + "px");
		           			}
		           			else {
