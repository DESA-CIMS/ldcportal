...
<%
	...
	if (mergeUrlTags || mergeLayoutTags) {
		...
		if (mergeLayoutTags) {
			...
			if (!layoutTagNames.isEmpty()) {
				compilerTagNames =
					ArrayUtil.append(compilerTagNames, layoutTagNames.toArray(new String[layoutTagNames.size()]));
			}
		}
	...
	for (String curAssetTagName : allAssetTagNames) {
		try {
			...
			AssetTagProperty journalTemplateIdProperty =
				AssetTagPropertyLocalServiceUtil.getTagProperty(assetTag.getTagId(), "journal-template-id");
	...
	if (enableTagBasedNavigation && selectionStyle.equals("manual") &&
		((assetEntryQuery.getAllCategoryIds().length > 0) || (assetEntryQuery.getAllTagIds().length > 0))) {
...
<c:if
	test="<%= (group != null) && (!group.hasStagingGroup() || group.isStagingGroup())  %>">
	<aui:form name="fm">
...
<c:if test="<%= showMetadataDescriptions %>">
	<c:choose>
		<c:when
			test='<%= (assetCategoryId > 0) && Validator.isNotNull(assetTagName) && selectionStyle.equals("dynamic") %>'>
			<h1 class="asset-categorization-title">
				<liferay-ui:message
					arguments="<%= new String[] {assetVocabularyTitle, assetCategoryTitle, assetTagName} %>"
					key="content-with-x-x-and-tag-x" />
			</h1>
		...
		<c:otherwise>
			<c:if test='<%= (assetCategoryId > 0) && selectionStyle.equals("dynamic") %>'>
				<h1 class="asset-categorization-title">
					<liferay-ui:message
						arguments="<%= new String[] {assetVocabularyTitle, assetCategoryTitle} %>"
						key="content-with-x-x" />
				</h1>
			...
			<c:if
				test='<%= Validator.isNotNull(assetTagName) && selectionStyle.equals("dynamic") %>'>
				<h1 class="asset-categorization-title">
...
<c:if test="<%= enableRSS %>">
	...
	<div class="subscribe">
		<liferay-ui:icon image="rss" label="<%= true %>" method="get" target="_blank"
			url="<%= rssURL %>" />
	</div>

	<liferay-util:html-top>
		<link href="<%=HtmlUtil.escape(rssURL)%>" rel="alternate" title="RSS"
...
<c:if test="<%= readMoreLink && !isMaximized && searchContainer.getTotal() > delta %>">
	<liferay-portlet:renderURL windowState="maximized" varImpl="portletMaximizedURL">
		<liferay-portlet:param name="ismKey" value="<%= String.valueOf(ismKey) %>" />
		<c:if test="<%= portletName.equals(PortletKeys.RELATED_ASSETS) %>">
			<liferay-portlet:param name="layoutAssetEntryId" value="<%= String.valueOf(layoutAssetEntryId) %>" />
		</c:if>
