...
<aui:script>
	function showInTextField(pageURL, layoutId) {
		$('#<portlet:namespace/>displayPage').val(pageURL);
		$('#<portlet:namespace/>assetLayoutId').val(layoutId);
	}
	...
	Liferay.provide(
		...
		function() {
			if ($('#<portlet:namespace/>displayPage').val() == "") {
				$('#<portlet:namespace/>assetLayoutId').val("");
			}
...
private void _buildLayoutView1(Layout layout, String cssClass, boolean useHtmlTitle, ThemeDisplay themeDisplay, StringBundler sb) throws Exception {
	String layoutURL = PortalUtil.getLayoutURL(layout, themeDisplay);
	String target = PortalUtil.getLayoutTarget(layout);
	boolean assetPublisherExists = false;
	try	{
		String assetViewId = "";
		LayoutTypePortlet ltportlet = (LayoutTypePortlet) layout.getLayoutType();
	...
	sb.append("<a href=\"#");
	//sb.append(layoutURL);
	if(assetPublisherExists) {
		sb.append("\" onclick=\"showInTextField(\'"+layoutURL+"\',\'"+ layout.getPlid()+ "\');\"");
	} else {
		sb.append("\" onclick=\"showMsgNoAssetPublisher();\"");
	}
	sb.append(target);
...
private void _buildSiteMap(Layout layout, List<Layout> layouts, Layout rootLayout, boolean includeRootInTree, int displayDepth, boolean showCurrentPage, boolean useHtmlTitle, boolean showHiddenPages, int curDepth, ThemeDisplay themeDisplay, StringBundler sb) throws Exception {
	...
	if (showRoot) {
		...
		_buildLayoutView1(rootLayout, cssClass, useHtmlTitle, themeDisplay, sb);
	...
	for (int i = 0; i < layouts.size(); i++) {
		...
		if ((showHiddenPages || !curLayout.isHidden()) && LayoutPermissionUtil.contains(permissionChecker, curLayout, ActionKeys.VIEW)) {
			...
			_buildLayoutView1(curLayout, cssClass, useHtmlTitle, themeDisplay, sb);
