...
long layoutAssetEntryId = GetterUtil.getLong(request.getParameter("layoutAssetEntryId"));

if (portletName.equals(PortletKeys.RELATED_ASSETS)) {
 	
	AssetEntry layoutAssetEntry = null;
	
	if (layoutAssetEntryId == GetterUtil.DEFAULT_LONG) {
		layoutAssetEntry = (AssetEntry)request.getAttribute(WebKeys.LAYOUT_ASSET_ENTRY);
	}
	else {
		layoutAssetEntry = AssetEntryLocalServiceUtil.getAssetEntry(layoutAssetEntryId);
	}
	
	if (layoutAssetEntry != null) {
		layoutAssetEntryId = layoutAssetEntry.getEntryId();
		
		assetEntryQuery.setLinkedAssetEntryId(layoutAssetEntry.getEntryId());
		
		if (ISMLocalServiceUtil.ismExistsByWebContendId(layoutAssetEntry.getEntryId())) {
	...
	// <-- ISM Hook
	//Get ISM and ISM-related category
	else {
		...
		if (ism != null) {
			assetEntryQuery.setLinkedAssetEntryId(ism.getWebContentId());		
		}	
	}
	// ISM Hook -->
}
...
boolean displayAssetItself = GetterUtil.getBoolean(preferences.getValue("displayAssetItself", null));
...
int deltaMaximized = GetterUtil.getInteger(preferences.getValue("deltaMaximized", StringPool.BLANK), SearchContainer.DEFAULT_DELTA);
boolean isMaximized = windowState.toString().equals(WindowState.MAXIMIZED.toString());
int abstractLengthMaximized = GetterUtil.getInteger(preferences.getValue("abstractLengthMaximized", null), 700);
boolean isStories = GetterUtil.getBoolean(preferences.getValue("isStories", null));

if (readMoreLink) {
	if (isMaximized) {
		delta = deltaMaximized;
		abstractLength = abstractLengthMaximized;
	}
	else {
		paginationType = "none";
	}
