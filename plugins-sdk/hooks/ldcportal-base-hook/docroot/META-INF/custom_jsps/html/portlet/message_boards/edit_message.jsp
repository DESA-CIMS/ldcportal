...
MBCategory mbCategory = MBCategoryLocalServiceUtil.getCategory(categoryId);
...
<liferay-ui:header
	...
	title='<%= (message == null) ? LanguageUtil.get(pageContext, "new-message") + " - " + mbCategory.getName() : message.getSubject() %>'
/>
...
<aui:form action="<%= editMessageURL %>" enctype='<%= attachments ? "multipart/form-data" : "" %>' method="post" name="fm" onSubmit='<%= "event.preventDefault(); " + renderResponse.getNamespace() + "saveMessage(false);" %>'>
	...
	<aui:fieldset>
		...
		<c:if test="<%= message == null %>">
			<aui:field-wrapper cssClass="new-permissions" label="permissions">
